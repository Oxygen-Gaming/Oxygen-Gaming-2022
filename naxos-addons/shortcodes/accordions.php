...
class Naxos_Shortcode_Accordions {
    ...
    public static function accordions( $atts, $content = null ) {
		...
		return '<div id="accordion' . self::$accordionsCounters . '" class="accordion" role="tablist" ' . $style . '>' . $content . '</div>';
	}
	...
	public static function accordion( $atts, $content = null ) {
		...
		$collapsed = ( $opened == 'false' ? 'collapsed' : '' );
		$show = ( $opened == 'true' ? 'show' : '' );
		
		return '<div class="accordion-item">
					<div class="accordion-header" id="heading-' . self::$accordionCounters . '">
						<h5><a class="accordion-button ' . $collapsed . '" href="#collapse-' . self::$accordionCounters . '" role="button" aria-expanded="' . $opened . '" data-bs-toggle="collapse" aria-controls="collapse-' . self::$accordionCounters . '">' . esc_html( $title ) . '</a></h5>
					</div>
					<div id="collapse-' . self::$accordionCounters . '" class="accordion-collapse collapse ' . $show . '" aria-labelledby="heading-' . self::$accordionCounters . '" data-bs-parent="#accordion' . self::$accordionsCounters . '">
