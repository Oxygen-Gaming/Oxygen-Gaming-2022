...
class Naxos_Init {	
	...
	public static function naxos_styles( ) {
		...
		if ( ! is_admin( ) ) {
			wp_enqueue_style( 'bootstrap', 					get_template_directory_uri( ) . '/assets/library/bootstrap/css/' . ( ! is_rtl( ) ? 'bootstrap.min.css' : 'bootstrap.rtl.min.css' ) );
	...
	public static function naxos_widgets( ) {
		...
		if ( ( $naxos_config['footer-column-width'] ) and class_exists( 'Naxos_Shortcodes' ) ) {
			$column_width = Naxos_Shortcodes::getColumnsNumber( $naxos_config['footer-column-width'] );
		}
		...
		register_sidebar( array(
			...
			'before_widget' => '<div id="%1$s" class="col-12 col-md-6 col-lg-' . esc_attr( $column_width ) . ' res-margin"><div class="widget %2$s">',
			'after_widget' 	=> '</div></div>',
			'before_title' 	=> '<h6>',
			'after_title' 	=> '</h6>',
		) );
